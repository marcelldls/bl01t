...
export EC_TARGET=b01-1-beamline
# the git repo for this project
export EC_SERVICES_REPO=git@github.com:marcelldls/bl01t.git
...
module unload k8s-b01-1 > /dev/null
module load k8s-b01-1 > /dev/null
# set the default namespace for kubectl and helm (for convenience only)
kubectl config set-context --current --namespace=b01-1-beamline
